# Một số điều cơ bản về React Native

## 1. Element cơ bản của React Native
`React Native` sử dụng các `component` để tạo ra giao diện người dùng. Các `component` cơ bản của React Native:
  - `View`: Component này tương tự như thẻ div trong HTML.
  - `Text`: Component này tương tự như thẻ p trong HTML.
  - `Image`: Component này tương tự như thẻ img trong HTML.
  - `TextInput`: Component này tương tự như thẻ input trong HTML.
  - `ScrollView`: Component này tương tự như thẻ div với thuộc tính overflow: scroll trong HTML.
  - `FlatList`: Component này tương tự như thẻ ul hoặc ol trong HTML.
  - `SectionList`: Component này tương tự như thẻ ul hoặc ol với các thẻ li trong HTML.
  - `Button`: Component này tương tự như thẻ button trong HTML.
  - `Pressable`: Component này tương tự như thẻ button trong HTML. (Nên)

## 2. Lưu ý khi sử dụng element trong React Native
Các element trong React Native `không hỗ trợ` tất cả các `thuộc tính của CSS`.

Các element trong React Native `có một số thuộc tính riêng` như:
  - `flexDirection`: Thuộc tính này giúp chúng ta xác định hướng hiển thị của các element con.
  - `justifyContent`: Thuộc tính này giúp chúng ta căn chỉnh vị trí của các element con theo chiều dọc.
  - `alignItems`: Thuộc tính này giúp chúng ta căn chỉnh vị trí của các element con theo chiều ngang.
  - `flex`: Thuộc tính này giúp chúng ta xác định kích thước của các element con.

Các element trong React Native `không hỗ trợ` các sự kiện như: `hover`, `focus`, act`ive.

Các element trong React Native `không hỗ trợ` các `pseudo-classes` như: :hover, :focus, :active.

Các element trong React Native `không hỗ trợ` các `pseudo-elements` như: ::before, ::after.

> Muốn sử dụng các thuộc tính CSS trong React Native, chúng ta cần sử dụng `StyleSheet.create()` để tạo ra một đối tượng chứa các style. Và sau đó, chúng ta sử dụng đối tượng này để gán style cho element thông qua `attribute style`. Hoặc có thể đổi style cho element theo kiểu `inline style` giống như làm với ReactJS.
```jsx
import { StatusBar } from "expo-status-bar";
import { StyleSheet, Text, View, Button } from "react-native"; // StyleSheet để tạo style cho element (Bắt buộc cần dùng trong React Native nếu muốn sử dụng CSS)

export default function App() {
  return (
    // Gán style cho element thông qua attribute style
    <View style={styles.container}> 
      <View>
        <Text style={styles.dummyText}>Hello World</Text>
      </View>
      <Text style={styles.dummyText}>Open up App.js to start working on your app!</Text>
      <StatusBar style="auto" />
      <Button title="Start" />
    </View>
  );
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: "#fff",
    alignItems: "center",
    justifyContent: "center",
  },
  dummyText: {
    borderWidth: 2,
    borderColor: "red",
    margin: 16,
    padding: 16,
  }
}); // Tạo style cho từng attribute
```

Trong React Native, chúng ta `không sử dụng các thẻ HTML` mà chúng ta sử dụng các `component của React Native` để tạo ra giao diện người dùng.

Muốn `in ra 1 chuỗi` trong React Native, chúng ta bắt buộc phải sử dụng component `<Text>`.

Muốn `hiển thị` cho người dùng `nhìn thấy được 1 component`, chúng ta bắt buộc phải sử dụng component `<View>`.

Đối với `<Button>` trong React Native, để thêm sự kiện click vào button, chúng ta sử dụng thuộc tính `onPress`. Để thêm text cho button, chúng ta sử dụng thuộc tính `title`. `<Button title='Start'/>` và thẻ `<Button>` trong React Native là 1 thẻ `self-closing tag`.

## 3. Flexbox trong React Native
Flexbox trong React Native giống với Flexbox trong CSS. Khác ở chỗ:
  - Trong React Native, muốn sử dụng Flexbox, chúng ta bắt buộc phải sử dụng component `<View>`. và không cần gọi ra thuộc tính `display: flex`.
  - Trong React Native, thuộc tính `flexDirection` mặc định là `column` chứ không phải là `row` như trong CSS.
  - Trong React Native, thuộc tính `flex` mặc định là `0` chứ không phải là `1` như trong CSS.
  - Trong React Native, thuộc tính `alignItems` mặc định là `stretch` chứ không phải là `flex-start` như trong CSS.
  - Trong React Native, thuộc tính `justifyContent` mặc định là `flex-start` chứ không phải là `center` như trong CSS.

## 4. Các thuộc tính xử lý sự kiện trong React Native
Trong React Native, chúng ta sử dụng các thuộc tính sau để xử lý sự kiện:
  - `onPress`: Sự kiện click chuột của `<Button>` hoặc `<TouchableOpacity>`.
  + `onLongPress`: Sự kiện giữ chuột của `<TouchableOpacity>`.
  - `onFocus`: Sự kiện focus vào element có thể áp dụng cho `<TextInput>`, `<TouchableOpacity>`.
  - `onBlur`: Sự kiện blur ra khỏi element có thể áp dụng cho `<TextInput>`, `<TouchableOpacity>`.
  - `onChangeText`: Sự kiện thay đổi text trong element có thể áp dụng cho `<TextInput>`.
  - `onSubmitEditing`: Sự kiện submit form của `<TextInput>`.
  - `onScroll`: Sự kiện scroll trên element có thể áp dụng cho `<ScrollView>`, `<FlatList>`, `<SectionList>`.
  - `onContentSizeChange`: Sự kiện thay đổi kích thước của element có thể áp dụng cho `<ScrollView>`, `<FlatList>`, `<SectionList>`.
  - `onEndEditing`: Sự kiện kết thúc chỉnh sửa element có thể áp dụng cho `<TextInput>`.
  - `onLayout`: Sự kiện thay đổi layout của element có thể áp dụng cho tất cả các element.

> Trong phiên bản React Native mới, chúng ta không cần phải import thư viện `TouchableOpacity,...` nữa, mà chúng ta có thể sử dụng `Pressable` để thay thế cho `TouchableOpacity,...`. `Pressable` là 1 component giúp chúng ta biến các component khác thành 1 component có thể bắt sự kiện. `Pressable` giúp chúng ta tối ưu hóa hiệu suất của ứng dụng.

> Mặc định trong `View` sẽ không có các thuộc tính xử lý sự kiện như `onPress`, `onLongPress`,... nên chúng ta cần phải sử dụng thêm component `Pressable` để có thể sử dụng các thuộc tính xử lý sự kiện. (Khác với `ReactJS` thì `div` có thể sử dụng các thuộc tính xử lý sự kiện như `onClick`, `onMouseOver`,... mà không cần phải bọc bên ngoài `Pressable`).
```jsx
import { StatusBar } from "expo-status-bar";
import { StyleSheet, Text, View, Button, Pressable } from "react-native";

export default function App() {
  return (
    <Pressable onPress={() => alert('Hello World')}> // Bọc bên ngoài View để sử dụng các thuộc tính xử lý sự kiện
      <View style={styles.container}>
        <View>
          <Text style={styles.dummyText}>Hello World</Text>
        </View>
        <Text style={styles.dummyText}>Open up App.js to start working on your app!</Text>
        <StatusBar style="auto" />
        <Button title="Start" />
      </View>
    </Pressable>
  );
}

```

### 4.1. Các thuộc tính của Pressable
Pressable có các thuộc tính giống như `TouchableOpacity`, ngoài ra còn có:
  - `android_ripple`: Hiệu ứng ripple trên Android, ví dụ: `android_ripple={{color: 'red'}}`. Thì khi click vào element sẽ có hiệu ứng ripple màu đỏ. Ngoài ra, trên `Android`, có thêm thuộc tính CSS là `evelation` để tạo hiệu ứng shadow cho button.
  - Trên `iOS`, `Pressable` sẽ không có hiệu ứng `ripple`. Tuy nhiên chúng ta có thể sử dụng `style` để tạo hiệu ứng tương tự. ví dụ:
  ```jsx
  <Pressable style={({pressed}) => [
    {
      backgroundColor: pressed ? 'red' : 'blue'
    }, // Khi click vào element, sẽ có hiệu ứng màu đỏ, ngược lại sẽ có hiệu ứng màu xanh
    styles.button
  ]}> // Các style sẽ được áp dụng khi click vào element, có thể áp dụng được nhiều style khi truyền vào mảng
    <Text style={styles.text}>Pressable</Text>
  </Pressable>
  ```
    + `pressed` là 1 biến boolean, nếu `pressed` là `true` thì sẽ có hiệu ứng màu đỏ, ngược lại sẽ có hiệu ứng màu xanh.
    + `hitSlop`: Tăng kích thước bắt sự kiện, ví dụ: `hitSlop={{top: 10, bottom: 10, left: 10, right: 10}}`. Khi click vào element, sẽ bắt sự kiện ở vùng rộng hơn 10px ở trên, dưới, trái, phải.
    + `onLongPress`: Sự kiện giữ chuột
    + `onPress`: Sự kiện click chuột
    + `onPressIn`: Sự kiện bắt đầu click chuột
    + `onPressOut`: Sự kiện kết thúc click chuột
    + `pressRetentionOffset`: Khoảng cách tối thiểu để bắt đầu click chuột, ví dụ: `pressRetentionOffset={{top: 10, bottom: 10, left: 10, right: 10}}`. Khi click vào element, sẽ bắt sự kiện ở vùng rộng hơn 10px ở trên, dưới, trái, phải.
    + `delayLongPress`: Thời gian giữ chuột, ví dụ: `delayLongPress={1000}`. Khi giữ chuột 1s thì sẽ bắt sự kiện.
    + `delayPressIn`: Thời gian bắt đầu click chuột, ví dụ: `delayPressIn={1000}`. Khi click chuột 1s thì sẽ bắt sự kiện.
    + `delayPressOut`: Thời gian kết thúc click chuột, ví dụ: `delayPressOut={1000}`. Khi kết thúc click chuột 1s thì sẽ bắt sự kiện.
    + `disabled`: Tắt sự kiện click chuột, ví dụ: `disabled={true}`. Khi click vào element sẽ không bắt sự kiện.
    + `android_disableSound`: Tắt âm thanh click chuột trên Android, ví dụ: `android_disableSound={true}`. Khi click vào element sẽ không có âm thanh.
    + `android_hasTVPreferredFocus`: Chọn element khi sử dụng TV remote trên Android, ví dụ: `android_hasTVPreferredFocus={true}`. Khi sử dụng TV remote, element này sẽ được chọn.
    + `tvParallaxProperties`: Hiệu ứng parallax trên TV, ví dụ: `tvParallaxProperties={{tvParallaxShiftDistanceX: 10, tvParallaxShiftDistanceY: 10}}`. Khi di chuyển element, sẽ có hiệu ứng parallax.
    + `tvParallaxShiftDistanceX`: Khoảng cách di chuyển parallax theo trục x trên TV


## 5. ScrollView trong React Native
`ScrollView` trong React Native giống với thẻ `div` với thuộc tính `overflow: scroll` trong HTML.

`ScrollView` trong React Native giúp chúng ta `cuộn nội dung` của element.

`Tạo ScrollView` để `cho phép` người dùng `cuộn nếu nội dung quá dài` khi vượt qua khỏi View cha. ScrollView sẽ tự động thêm thanh cuộn nếu nội dung vượt quá View cha

Thêm thuộc tính `alwaysBounceVertical={false}` để tắt chức năng cuộn khi kéo xuống dưới cùng hoặc kéo lên trên cùng khi nội dung không vượt quá View cha

Ngoài ra ScrollView còn có các thuộc tính khác như 
  - `horizontal={true}` để cho phép cuộn ngang
  - `showsHorizontalScrollIndicator={false}` để tắt thanh cuộn ngang
  - `showsVerticalScrollIndicator={false}` để tắt thanh cuộn dọc
  - `contentContainerStyle={{padding: 16}}` để thêm style cho nội dung bên trong ScrollView
  - `pagingEnabled={true}` để cuộn theo từng trang
  - `onScroll={handleScroll}` để xử lý sự kiện cuộn
  - `scrollEventThrottle={16}` để giảm tần suất sự kiện cuộn
  - `onMomentumScrollBegin={handleMomentumScrollBegin}` để xử lý sự kiện bắt đầu cuộn
  - `onMomentumScrollEnd={handleMomentumScrollEnd}` để xử lý sự kiện kết thúc cuộn
  - `onContentSizeChange={handleContentSizeChange}` để xử lý sự kiện thay đổi kích thước nội dung
  - `onScrollBeginDrag={handleScrollBeginDrag}` để xử lý sự kiện bắt đầu kéo
  - `onScrollEndDrag={handleScrollEndDrag}` để xử lý sự kiện kết thúc kéo
  - `scrollEnabled={false}` để tắt chức năng cuộn
  - `scrollEventThrottle={16}` để giảm tần suất sự kiện cuộn
  - `snapToInterval={200}` để cuộn theo từng khoảng cách cố định
  - `snapToAlignment="center"` để căn giữa khi cuộn
  - `snapToStart={true}` để cuộn về đầu
  - `snapToEnd={true}` để cuộn về cuối
  - `decelerationRate="fast"` để tăng tốc độ cuộn
  - `bounces={false}` để tắt bounce effect
  - `bouncesZoom={false}` để tắt bounce zoom effect
  - `maximumZoomScale={2}` để thiết lập zoom tối đa
  - `minimumZoomScale={0.5}` để thiết lập zoom tối thiểu


## 6. FlatList trong React Native thay thế cho ScrollView 
`FlatList` là 1 `self-closed tag` component.

`FlatList` trong React Native `giống với thẻ ul hoặc ol` trong HTML.

`FlatList` trong React Native giúp chúng ta `hiển thị danh sách các phần tử.`

`FlatList` trong React Native giúp chúng ta `tối ưu hóa` hiển thị `danh sách các phần tử` nhờ chức năng `lazy loading`.

### 6.1. FlatList vs ScrollView
Khi muốn hiển thị danh sách các phần tử, chúng ta nên sử dụng FlatList thay vì ScrollView vì:
  - `FlatList` giúp chúng ta `tối ưu hóa hiển thị` danh sách các phần tử.
  - `FlatList` giúp chúng ta `hiển thị danh sách` các phần tử mà `không cần phải render tất cả` các phần tử.
  - `FlatList` giúp chúng ta `hiển thị danh sách` các phần tử mà `không cần phải tạo ra nhiều component`.
  - `Sử dụng lazy loading` để tải dữ liệu khi cần thiết giúp `tăng tốc độ hiển thị` danh sách các phần tử.

### 6.2. Các thuộc tính của FlatList
`FlatList` hầu như có các thuộc tính giống như ScrollView, ngoài ra còn có:
  - `data`: Mảng dữ liệu cần hiển thị, truyền vào mảng dữ liệu. (**LƯU Ý: PHẢI LÀ KIỂU MẢNG**)
  - `renderItem`: `Component` hiển thị mỗi phần tử trong mảng dữ liệu (**(item) => Component(item)**)
  - `keyExtractor`: Hàm trả về key cho mỗi phần tử trong mảng dữ liệu ((item, index) => index), với index là vị trí của phần tử trong mảng dữ liệu.
  - `horizontal`: Hiển thị danh sách ngang
  - `numColumns`: Số cột hiển thị
  - `columnWrapperStyle`: Style cho mỗi cột
  - `ListHeaderComponent`: Component hiển thị ở đầu danh sách
  - `ListFooterComponent`: Component hiển thị ở cuối danh sách
  - `ListEmptyComponent`: Component hiển thị khi danh sách rỗng
  - `ItemSeparatorComponent`: Component hiển thị giữa các phần tử
  - `refreshing`: Trạng thái tải lại dữ liệu
  - `onRefresh`: Sự kiện tải lại dữ liệu
  - `onEndReached`: Sự kiện cuộn đến cuối danh sách
  - `onEndReachedThreshold`: Khoảng cách cuộn đến cuối danh sách
  - `initialNumToRender`: Số lượng phần tử hiển thị ban đầu
  - `maxToRenderPerBatch`: Số lượng phần tử hiển thị mỗi lần render
  - `windowSize`: Số lượng phần tử hiển thị trên màn hình
  - `getItemLayout`: Hàm trả về layout cho mỗi phần tử
  - `removeClippedSubviews`: Tắt chức năng hiển thị phần tử ngoài màn hình
  - `legacyImplementation`: Sử dụng cách hiển thị cũ
  - `viewabilityConfig`: Cấu hình hiển thị phần tử
  - `viewabilityConfigCallbackPairs`: Cấu hình hiển thị phần tử

### 6.3. Ví dụ về FlatList
```jsx
...
import { FlatList } from "react-native";
...
<FlatList
  showsVerticalScrollIndicator={false} // Tắt thanh cuộn dọc
  alwaysBounceVertical={false} // Tắt chức năng cuộn khi kéo xuống dưới cùng hoặc kéo lên trên cùng khi nội dung không vượt quá View cha
  data={logGuess} // Mảng dữ liệu cần hiển thị
  keyExtractor={(data, index) => index} // Hàm trả về key cho mỗi phần tử trong mảng dữ liệu, data là `logGuess`, index là vị trí của phần tử trong mảng dữ liệu
  renderItem={(item) => ( // Component hiển thị mỗi phần tử trong mảng dữ liệu
    <GuestLogItem guestNumber={item.item} roundNumber={item.index} /> 
  )}
  /*
  item = {
    item: 1,
    index: 0
  }
  */
/>
...
```

## 7. Modal trong React Native
`Modal` trong React Native giúp chúng ta `hiển thị 1 component lớn hơn đè lên trên component hiện tại` nó đang bao bọc.

`Modal` trong React Native giúp chúng ta `tạo ra 1 popup` có thể đóng hoặc mở.

### 7.1. Các thuộc tính của Modal
Modal có các thuộc tính giống như các component khác, ngoài ra còn có:
  - `animationType`: Loại animation hiển thị modal, ví dụ: `animationType="slide"`. Có thể là `none`, `slide`, `fade`.
  - `transparent`: Hiển thị modal trong suốt, ví dụ: `transparent={true}`.
  - `visible`: Hiển thị modal hay không, ví dụ: `visible={true}`.
  - `onRequestClose`: Sự kiện khi đóng modal, ví dụ: `onRequestClose={() => alert('Close')}`.
  - `onShow`: Sự kiện khi hiển thị modal, ví dụ: `onShow={() => alert('Show')}`.
  - `statusBarTranslucent`: Hiển thị status bar trong suốt, ví dụ: `statusBarTranslucent={true}`.
  - `supportedOrientations`: Hướng màn hình hỗ trợ, ví dụ: `supportedOrientations={['portrait', 'landscape']}`.
  - `presentationStyle`: Kiểu hiển thị modal, ví dụ: `presentationStyle="pageSheet"`. Có thể là `fullScreen`, `pageSheet`, - `formSheet`, `overFullScreen`.
  - `hardwareAccelerated`: Sử dụng hardware để tăng tốc độ hiển thị, ví dụ: `hardwareAccelerated={true}`.
  - `onDismiss`: Sự kiện khi đóng modal, chọn huỷ, ví dụ: `onDismiss={() => alert('Dismiss')}`.
  - `onOrientationChange`: Sự kiện khi thay đổi hướng màn hình, ví dụ: `onOrientationChange={() => alert('Orientation Change')}`.
  - `onMagicTap`: Sự kiện khi double tap status bar, ví dụ: `onMagicTap={() => alert('Magic Tap')}`.
  - `pointerEvents`: Sự kiện click chuột, ví dụ: `pointerEvents="box-none"`. Có thể là `auto`, `none`, `box-none`, - `box-only`.

### 7.2. Ví dụ về Modal
```jsx
...
import { Modal } from "react-native";
...
// useState, function liên quan đến showModal (true|false) - đóng mở Modal, handleTextInput - xử lý input, handleAddCourse - thêm course, handleHideModal - thay đổi trạng thái của showModal

<Modal visible={showModal} animationType="slide">
  {/* Tạo Modal, visible để hiển thị hoặc ẩn Modal, animationType để thiết lập kiểu hiệu ứng khi hiển thị Modal */}
  <View style={styles.inputContainer}>
    <Image
      style={styles.image}
      source={require("../assets/images/goal.png")}
    />
    <TextInput
      style={styles.textInput}
      placeholder="Your course goal"
      onChangeText={handleTextInput}
      value={courseGoal}
    />
    {/* Tạo input, trong RN sẽ là self close tag */}
    <View style={styles.buttonContainer}>
      <View style={styles.button}>
        <Button title="Cancel" onPress={handleHideModal} color='#f31282'/>
      </View>
      <View style={styles.button}>
        <Button
          title="Add goal"
          onPress={() => {
            handleAddCourse(courseGoal);
            setCourseGoal("");
          }}
          color='#b180f0'
        />
      </View>
    </View>

    {/* Tạo button, trong RN sẽ là self close tag, Button trong RN sẽ không có thẻ style */}
  </View>
</Modal>
...
```

## 8. Image và ImageBackground trong React Native
### 8.1. Image
`Image` trong React Native giống với thẻ `img` trong HTML.

Trong React Native, để `dẫn đường dẫn của ảnh`, chúng ta cần phải sử dụng thuộc tính `source`. Có 2 cách truyền link ảnh vào sau:
- Sử dụng `require` để `import` ảnh vào `project`. Và đường dẫn ảnh là `đường dẫn tương đối`. (Nếu ảnh nằm trong máy cục bộ). Ví dụ `source={require('./assets/logo.png')}`.
- Sử dụng `uri` để `truyền link ảnh`. (Nếu ảnh nằm trên mạng). Ví dụ `source={{uri: 'https://example.com/logo.png'}}`.

Ví dụ: `<Image source={require('./assets/logo.png')} />`.

### 8.2. ImageBackground
`ImageBackground` trong React Native giống với thuộc tính `background-image` trong CSS.

Trong React Native,bọc các component bên trong `<ImageBackground>` để sử dụng `ImageBackground`.

`ImageBackground` có các attribute giống như `Image`

Trong React Native, để `dẫn đường dẫn của ảnh`, chúng ta cần phải sử dụng thuộc tính `source`. Có 2 cách truyền link ảnh vào sau:
- Sử dụng `require` để `import` ảnh vào `project`. Và đường dẫn ảnh là `đường dẫn tương đối`. (Nếu ảnh nằm trong máy cục bộ). Ví dụ `source={require('./assets/logo.png')}`.
- Sử dụng `uri` để `truyền link ảnh`. (Nếu ảnh nằm trên mạng). Ví dụ `source={{uri: 'https://example.com/logo.png'}}`.

Ngoài ra để style ảnh nền, chúng ta sử dụng thuộc tính `imageStyle` và truyền vào `StyleSheet.create()` để tạo ra 1 đối tượng chứa các style. Lúc này style của ảnh chính là thẻ `style` của `Image`

Ví dụ:
```jsx
<ImageBackground
  source={require("./assets/images/background.png")} // ImageBackground source
  style={styles.container} // ImageBackground style
  resizeMode="cover" // imageResizeMode
  imageStyle={styles.image} // imageStyle cho ảnh nền
>
  <View style={styles.overlay}></View>
  <Text style={styles.text}>Hello World</Text>
</ImageBackground>
```

## 9. StatusBar trong React Native (thư viện expo-status-bar)
`StatusBar` trong React Native giống với thẻ `meta` trong HTML.

`StatusBar` trong React Native giúp chúng ta tùy chỉnh màu sắc, hiển thị, ẩn thanh trạng thái của ứng dụng.

Các thuộc tính của `StatusBar`:
  - `style`: Style của thanh trạng thái, ví dụ: `style="auto"`. Có thể là `auto`, `light`, `dark`, `inverted`.
  - `backgroundColor`: Màu nền của thanh trạng thái, ví dụ: `backgroundColor="red"`.
  - `translucent`: Hiển thị thanh trạng thái trong suốt, ví dụ: `translucent={true}`.
  - `hidden`: Ẩn thanh trạng thái, ví dụ: `hidden={true}`.
  - `animated`: Hiệu ứng khi thay đổi thanh trạng thái, ví dụ: `animated={true}`.
  - `networkActivityIndicatorVisible`: Hiển thị biểu tượng mạng, ví dụ: `networkActivityIndicatorVisible={true}`.
  - `showHideTransition`: Hiệu ứng khi ẩn hiện thanh trạng thái, ví dụ: `showHideTransition="fade"`. Có thể là `fade`, `slide`.

Bằng tạo `<StatusBar/>`, chúng ta có thể tùy chỉnh màu sắc, hiển thị, ẩn thanh trạng thái của ứng dụng.
```jsx
import { StatusBar } from "expo-status-bar";

export default function App() {
  return (
    <StatusBar style="auto" />
    <View style={styles.container}>
      <Text>Hello World</Text>
    </View>
  );
}
```

## 10. Alert trong React Native
`Alert` trong React Native giống với `alert` trong JavaScript.

`Alert` trong React Native giúp chúng ta hiển thị thông báo cho người dùng.

`Alert` trong React Native có 2 phương thức:
  + `Alert.alert()`: Hiển thị thông báo cơ bản.
  + `Alert.prompt()`: Hiển thị thông báo có input.

`Alert.alert()` có 3 tham số:
  - 2 tham số đầu là 2 chuỗi: `title` thể hiện tiêu đề của thông báo và `message` thể hiện nội dung của thông báo.
  - `buttons`: Mảng các button của thông báo. Mỗi button là 1 object có 3 thuộc tính: `text`, `style` và `onPress`. Ví dụ: `buttons: [{text: 'OK', style: 'destructor' onPress: () => alert('OK')}])`.

`Alert.prompt()` có 4 tham số:
  - `2 tham số đầu là 2 chuỗi: `title` thể hiện tiêu đề của thông báo và `message` thể hiện nội dung của thông báo.
  - `callback`: Hàm callback trả về giá trị của input.
  - `type`: Loại input, ví dụ: `type: 'plain-text'`. Có thể là `plain-text`, `secure-text`, `numeric`, `email-address`, `phone-pad`.


Ví dụ:
>> Alert.alert()
```jsx
import { Alert } from "react-native";

export default function App() {
  return (
    <View style={styles.container}>
      <Button title="Alert" onPress={() => Alert.alert('Title', 'Message', [{text: 'OK', style: 'destructive', onPress: () => alert('OK')}])} />
    </View>
  );
}
```

>> Alert.prompt()
```jsx
import { Alert } from "react-native";

export default function App() {
  return (
    <View style={styles.container}>
      <Button title="Prompt" onPress={() => Alert.prompt('Title', 'Message', (text) => alert(text), 'plain-text')} />
    </View>
  );
}
```

## 11. SafeAreaView trong React Native
`SafeAreaView` trong React Native giống với `safe-area-view` trong CSS.

`SafeAreaView` trong React Native giúp chúng ta hiển thị component trong vùng an toàn của thiết bị. Ví dụ: không bị che bởi thanh trạng thái, thanh điều hướng, tai thỏ, notch của thiết bị.

`SafeAreaView` chỉ hỗ trợ các thiết bị IOS

Ví dụ:
```jsx
import { SafeAreaView } from "react-native";

export default function App() {
  return (
    <SafeAreaView style={styles.container}>
      <Text>Hello World</Text>
    </SafeAreaView>
  );
}
```

## 12. Sử dụng `Dimensions API` trong React Native để xác định kích thước thiết bị (tương tự như Media Query trong CSS)
`Dimentions API` là một API cung cấp thông tin về kích thước của thiết bị. Nó cho phép bạn lấy thông tin về kích thước của thiết bị, kích thước của cửa sổ, và nhiều hơn nữa.

Để sử dụng `Dimentions API` trong `React Native`, bạn cần import `Dimention` từ `react-native` như sau:
```javascript
import { Dimensions } from 'react-native';
```

Sau đó, bạn có thể sử dụng `Dimention API` để lấy thông tin về kích thước của thiết bị như sau:
```javascript
const { width, height } = Dimensions.get('window');
```

Có 2 loại kích thước mà bạn có thể lấy thông tin từ `Dimention API`:
- `window`: Kích thước của cửa sổ hiển thị ứng dụng. (Không tính thanh trạng thái, thanh điều hướng, tai thỏ, notch)
- `screen`: Kích thước của màn hình thiết bị. (Tính cả thanh trạng thái, thanh điều hướng, tai thỏ, notch)

Sau khi `get` theo `window` hoặc `screen`, bạn sẽ nhận được một object chứa thông tin về kích thước của thiết bị như sau:
```javascript
{
  width: 375,
  height: 812,
  scale: 2,
  fontScale: 2
}
```

> Sau khi lấy thông tin về kích thước của thiết bị, bạn có thể sử dụng nó để tạo các câu điều kiện CSS tương tự như Media Query trong CSS.

### 12.1. Sử dụng `useWindowDimensions` hook trong React Native
`useWindowDimensions` là một hook giúp bạn lấy thông tin về kích thước của cửa sổ hiển thị ứng dụng. Nó giống như `Dimensions.get('window')` nhưng nó sẽ tự động cập nhật khi kích thước của cửa sổ thay đổi. (Ví dụ: xoay màn hình, thay đổi kích thước cửa sổ)

Bởi vì, khi sử dụng `Dimensions.get('window')`, kích thước lấy được lần đầu sẽ là cố định, khi thay đổi kích thước cửa sổ, kích thước lấy được sẽ không thay đổi.

Sử dụng `useWindowDimensions` vào trong `Component` mỗi khi kích thước cửa sổ thay đổi, `Component` sẽ tự động render lại với kích thước mới.

Để sử dụng `useWindowDimensions` trong `React Native`, bạn cần import `useWindowDimensions` từ `react-native` như sau:
```javascript
import { useWindowDimensions } from 'react-native';

const Screen = () => {
  const { width, height } = useWindowDimensions();

  return (
    <View style={{ width, height }}>
      <Text>Hello World</Text>
    </View>
  );
}
```
### 12.2. Sử dụng `KeyboardAvoidingView` + `ScrollView` trong React Native.
Trong ứng dụng `React Native`, khi bàn phím hiển thị, nó có thể che phủ lên các phần tử khác trong ứng dụng. Để tránh tình trạng này, bạn có thể sử dụng `KeyboardAvoidingView` kết hợp với `ScrollView`.

`KeyboardAvoidingView` giúp chúng ta tránh việc bàn phím che phủ lên các phần tử khác trong ứng dụng. `ScrollView` giúp chúng ta cuộn nội dung khi bàn phím hiển thị, tránh làm các phần tử bị co lại.

Để sử dụng `KeyboardAvoidingView` + `ScrollView` trong `React Native`, bạn cần import `KeyboardAvoidingView` và `ScrollView` từ `react-native` như sau:
```javascript
import { KeyboardAvoidingView, ScrollView } from 'react-native';
const Input = () => {
  return (
    <ScrollView style={{ flex: 1 }}>
      <KeyboardAvoidingView behavior="position" style={{ flex: 1 }}> 
        <TextInput placeholder="Enter your name" />
      </KeyboardAvoidingView>
    </ScrollView>
  )
}
```
> Trong Component `KeyboardAvoidingView` thường sử dụng `behavior="position"` để di chuyển phần tử lên trên khi bàn phím hiển thị.

### 12.3. Sử dụng `Platform` API trong React Native để xác định hệ điều hành của thiết bị
`Platform` API là một API cung cấp thông tin về hệ điều hành của thiết bị. Nó cho phép bạn xác định hệ điều hành của thiết bị là `IOS` hay `Android`.

Các thuộc tính của `Platform` API:
- `OS`: Hệ điều hành của thiết bị. Có thể là `ios`, `android`, `web`, `windows`, `macos`, `default`.
- `Version`: Phiên bản hệ điều hành của thiết bị. Ví dụ: `10.0`, `11.0`, `12.0`.
- `select`: Chọn giá trị dựa trên hệ điều hành. Ví dụ: `Platform.select({ios: 'IOS', android: 'Android'})`.
- `is`: Kiểm tra hệ điều hành. Ví dụ: `Platform.isIOS`, `Platform.isAndroid`.

Ví dụ, đổi CSS dựa trên hệ điều hành:
```javascript
import { Platform } from 'react-native';

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: Platform.OS === 'ios' ? 'blue' : 'red' 
    // Hoặc
    backgroundColor: Platform.select({ios: 'blue', android: 'red'})
  }
});
```

### 12.4. Đặt tên file tương ứng với hệ điều hành trong React Native
Bình thường đối với các component sẽ có tên dạng `[name].js` hoặc `[name].jsx`. Tuy nhiên, trong `React Native`, bạn có thể đặt tên file tương ứng với hệ điều hành.

Để đặt tên file tương ứng với hệ điều hành trong `React Native`, bạn cần thêm hậu tố `.[platform]` vào sau tên file. Ví dụ: `App.ios.js`, `App.android.js`.

Lúc này, khi `import` vào một Component khác, ta chỉ cần import tên file gốc mà không cần phải quan tâm đến hệ điều hành. `React Native` sẽ tự động chọn file tương ứng với hệ điều hành.

Điều này giúp chúng ta tách biệt code giữa các hệ điều hành, giúp dễ dàng quản lý và bảo trì code hơn.

Ví dụ:
> `Color.ios.js`:
```javascript
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: 'blue'
  }
});
```
> `Color.android.js`:
```javascript
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: 'red'
  }
});
```

> `App.js`:
```javascript
import Color from './Color';
... 
// Lúc này, khi import Color vào App.js, React Native sẽ tự động chọn file tương ứng với hệ điều hành.
```

## 13. Sử dụng `TextInput` xử lý input trong React Native, và validate input
`TextInput` trong React Native giống với thẻ `input` trong HTML.

Tham khảo thêm về `TextInput` tại: [TextInput trong React Native](https://reactnative.dev/docs/textinput)

### 13.1. Các thuộc tính cơ bản của thẻ `TextInput`
- `autoCapitalize`: Tự động viết hoa chữ cái đầu tiên của mỗi từ.
  + `none`: Không viết hoa.
  + `sentences`: Viết hoa chữ cái đầu tiên của mỗi câu.
  + `words`: Viết hoa chữ cái đầu tiên của mỗi từ.
  + `characters`: Viết hoa tất cả chữ cái.
- `autoCorrect`: Tự động sửa lỗi chính tả. Mặc định sẽ được `bật`, để tắt, bạn cần set `autoCorrect={false}`.
- `autoComplete`: Tự động hoàn thành từ. Mặc định sẽ được `bật`, để tắt, bạn cần set `autoComplete={off}`.
- `editable`: Cho phép chỉnh sửa nội dung của `TextInput`. Mặc định sẽ được `bật`, để không cho phép chỉnh sửa, bạn cần set `editable={false}`.
- `keyboardType`: Nó xác định kiểu bàn phím hiển thị khi người dùng tương tác với `TextInput`.
  + `default`: Bàn phím mặc định.
  + `number-pad`: Bàn phím số.
  + `decimal-pad`: Bàn phím số với dấu chấm.
  + `numeric`: Bàn phím số.
  + `email-address`: Bàn phím email.
  + `phone-pad`: Bàn phím số điện thoại.
  + `url`: Bàn phím URL.
  + `visible-password`: Bàn phím hiển thị mật khẩu (Chỉ có trên Android).
- `maxLength`: Số ký tự tối đa cho phép nhập vào.
- `multiline`: Mặc định là `false`, để cho phép nhập nhiều dòng, bạn cần set `multiline={true}`. Nó sẽ hoạt động giống như thẻ `textarea` trong HTML.
- `onChangetext`: Sự kiện xảy ra khi người dùng nhập giá trị vào TextInput. Ví dụ: `onChangetext={(text) => console.log(text)}`.
- `placeholder`: Hiển thị placeholder trong `TextInput`.
- `placeholderTextColor`: Màu sắc của placeholder.
- `readOnly`: Chỉ cho phép đọc nội dung của `TextInput`. Mặc định sẽ được `tắt`, để bật, bạn cần set `readOnly={true}`.
- `textAlign`: Căn chỉnh văn bản trong `TextInput`. Có thể là `left`, `right`, `center`.
- `value`: Dùng để lấy giá trị, hoặc set giá trị trong trường `TextInput`.

### 13.2. Validate input trong `TextInput`
Để validate input trong `TextInput`, bạn có thể sử dụng `state` để lưu giá trị của `TextInput`, và sử dụng `useState` để cập nhật giá trị của `TextInput`.

Sau đó, bạn có thể sử dụng `if else` hoặc `switch case` để kiểm tra giá trị của `TextInput` và hiển thị thông báo lỗi.

Ví dụ:
```jsx
import { useState } from 'react';
import { View, TextInput, Button, Alert } from 'react-native';

const App = () => {
  const [text, setText] = useState('');

  const handleValidate = () => {
    if (text === '') {
      Alert.alert('Error', 'Please enter your name');
    } else {
      Alert.alert('Success', 'Your name is ' + text);
    }
  }

  return (
    <View>
      <TextInput
        placeholder="Enter your name"
        onChangeText={setText}
        value={text}
      />
      <Button title="Submit" onPress={handleValidate} />
    </View>
  );
}
```

## 14. Sử dụng `ActivityIndicator` trong React Native
`ActivityIndicator` trong React Native giống với `spinner` trong HTML. Dùng để hiện `loading` khi đang tải dữ liệu.

Các thuộc tính của `ActivityIndicator`:
- `animating`: Hiển thị `loading` hay không. Mặc định sẽ là `true`, để tắt, bạn cần set `animating={false}`.
- `color`: Màu sắc của `loading`.
- `size`: Kích thước của `loading`. Có thể là `small`, `large`.

Ví dụ:
```jsx
import { View, StyleSheet, ActivityIndicator } from "react-native";

function Loading() {
  return (
    <View style={styles.container}>
      <ActivityIndicator size="large" color="white" />
    </View>
  );
}

export default Loading;

const styles = StyleSheet.create({
  container: {
    flex: 1,
    alignItems: "center",
    justifyContent: "center",
  },
});
```

## 15. Tự tạo `Error` component trong React Native để xử lý Request API bị lỗi
Khi sử dụng `API` trong ứng dụng `React Native`, có thể xảy ra lỗi khi request API. Để xử lý lỗi này, bạn có thể tạo `Error` component để hiển thị thông báo lỗi.

`Error` component sẽ hiển thị thông báo lỗi khi request API bị lỗi. Bạn có thể tùy chỉnh thông báo lỗi, màu sắc, icon, nút `Refresh` để tải lại dữ liệu.

Ví dụ:
>Error.js
```jsx
import { StyleSheet, View, Text } from "react-native";
import Button from "./Button";
import { GlobalStyles } from "../../constants/styles";

function ErrorOverlay({ message, onPress }) {
  return (
    <View style={styles.container}>
      <Text style={[styles.text, styles.title]}>An Error Occurred!</Text>
      <Text style={styles.text}>{message}</Text>
      <Button onPress={onPress}>OK</Button>
    </View>
  );
}

export default ErrorOverlay;
const styles = StyleSheet.create({
  container: {
    flex: 1,
    padding: 24,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: GlobalStyles.colors.primary700,
  },
  text: {
    color: "white",
    textAlign: "center",
    marginBottom: 8,
  },
  title: {
    fontSize: 20,
    fontWeight: "bold",
  },
});
```

Sau đó, đặt `try-catch` trong hàm `fetch` để bắt lỗi khi request API bị lỗi. Khi request API bị lỗi, hiển thị `Error` component.

Ví dụ:
```jsx
import { useState } from "react";
import { View, Text, Button } from "react-native";

const App = () => {
  const [data, setData] = useState(null);
  const [error, setError] = useState(null);

  const handleFetchData = async () => {
    try {
      const response = await fetch("https://api.example.com/data");
      const data = await response.json();
      setData(data);
    } catch (error) {
      setError(error.message);
    }
  };

  if (error) {
    return <Error message={error} onPress={handleFetchData} />;
  }

  return (
    <View>
      <Text>{data}</Text>
      <Button title="Fetch Data" onPress={handleFetchData} />
    </View>
  );
};
```

## 16. Lưu trữ dữ liệu ở Storage của thiết bị trong React Native
Trong ứng dụng `React Native`, bạn có thể lưu trữ dữ liệu ở `Storage` của thiết bị. Giống như sử dụng `LocalStorage` để lưu trữ dữ liệu trong trình duyệt.

Có các thư viện hỗ trợ lưu trữ dữ liệu ở `Storage` của thiết bị trong `React Native`:
- `@react-native-async-storage/async-storage`: Thư viện thay thế cho `AsyncStorage`. (Hỗ trợ từ `React Native 0.64` trở lên)
- `react-navite-mmkv`: Thư viện hỗ trợ lưu trữ dữ liệu ở `Storage` của thiết bị. (Nhanh hơn `AsyncStorage`)

> LƯU Ý: NẾU LƯU `TOKEN` HOẶC `PASSWORD` TRONG STORAGE, BẠN CẦN MÃ HÓA DỮ LIỆU TRƯỚC KHI LƯU. SỬ DỤNG THƯ VIỆN `SecureStore` THAY VÌ `AsyncStorage`.

### 16.1. Sử dụng `@react-native-async-storage/async-storage` để lưu trữ dữ liệu ở Storage của thiết bị
`@react-native-async-storage/async-storage` là một thư viện hỗ trợ lưu trữ dữ liệu ở `Storage` của thiết bị trong `React Native`.

Để sử dụng `@react-native-async-storage/async-storage` trong `React Native`, bạn cần cài đặt thư viện `@react-native-async-storage/async-storage` như sau:
```bash
npm install @react-native-async-storage/async-storage
```

Sau đó, bạn cần import `AsyncStorage` từ `@react-native-async-storage/async-storage` như sau:
```javascript
import AsyncStorage from '@react-native-async-storage/async-storage';
```

Các phương thức của `AsyncStorage`:
- `setItem`: Lưu trữ dữ liệu vào `Storage`. Ví dụ: `AsyncStorage.setItem('key', 'value')`.
- `getItem`: Lấy dữ liệu từ `Storage`. Ví dụ: `AsyncStorage.getItem('key')`.
- `removeItem`: Xóa dữ liệu từ `Storage`. Ví dụ: `AsyncStorage.removeItem('key')`.
- `clear`: Xóa tất cả dữ liệu trong `Storage`. Ví dụ: `AsyncStorage.clear()`.
- `multiSet`: Lưu trữ nhiều dữ liệu vào `Storage`. Ví dụ: `AsyncStorage.multiSet([['key1', 'value1'], ['key2', 'value2']])`.
- `multiGet`: Lấy nhiều dữ liệu từ `Storage`. Ví dụ: `AsyncStorage.multiGet(['key1', 'key2'])`.

> LƯU Ý: TÂT CẢ CÁC PHƯƠNG THỨC CỦA `ASYNCSTORAGE` ĐỀU TRẢ VỀ `PROMISE`.
Ví dụ:
```jsx
import { useState } from "react";
import { View, Text, Button } from "react-native";

const App = () => {
  const [data, setData] = useState(null);

  const handleSetData = async () => {
    await AsyncStorage.setItem('key', 'value');
  };

  const handleGetData = async () => {
    const data = await AsyncStorage.getItem('key');
    setData(data);
  };

  const handleRemoveData = async () => {
    await AsyncStorage.removeItem('key');
  };

  return (
    <View>
      <Text>{data}</Text>
      <Button title="Set Data" onPress={handleSetData} />
      <Button title="Get Data" onPress={handleGetData} />
      <Button title="Remove Data" onPress={handleRemoveData} />
    </View>
  );
};
```

### 16.2. Sử dụng `react-native-mmkv` để lưu trữ dữ liệu ở Storage của thiết bị
`react-native-mmkv` là một thư viện hỗ trợ lưu trữ dữ liệu ở `Storage` của thiết bị trong `React Native`. Nó nhanh hơn `AsyncStorage`.

Để sử dụng `react-native-mmkv` trong `React Native`, bạn cần cài đặt thư viện `react-native-mmkv` như sau:
```bash
npm install react-native-mmkv
```

Cần import `MMKV` từ `react-native-mmkv` như sau:
```javascript
import MMKV from 'react-native-mmkv';
```

Sau đó, bạn cần khởi tạo `MMKV` như sau:
```javascript
const mmkv = new MMKV();
// Hoặc
const mmkv = new MMKV({
  id: 'myApp',
  encryptionKey: 'password',
  path: '/path/to/file',
});
```

Các phương thức của `MMKV`:
- `set`: Lưu trữ dữ liệu vào `Storage`. Ví dụ: `mmkv.set('key', 'value')`. Lưu ý: Dữ liệu truyền vào phải được chuyển sang `JSON` nếu là `Object`.
- `getString`: Lấy dữ liệu String từ `Storage`. Ví dụ: `mmkv.getString('key')`. Lưu ý cần phải chuyển đổi dữ liệu từ `JSON` nếu là `Object` với `JSON.parse()`.
- `getNumber`: Lấy dữ liệu từ Số `Storage`. Ví dụ: `mmkv.getNumber('key')`.
- `getBoolean`: Lấy dữ liệu từ Boolean `Storage`. Ví dụ: `mmkv.getBoolean('key')`.
- `contains`: Kiểm tra xem `key` có tồn tại trong `Storage` hay không. Ví dụ: `mmkv.contains('key')`.
- `getAllKeys`: Lấy tất cả các `key` trong storage. Ví dụ: `mmkv.getAllKeys()`.
- `delete`: Xóa dữ liệu từ `Storage`. Ví dụ: `mmkv.delete('key')`.
- `clearAll`: Xóa tất cả dữ liệu trong `Storage`. Ví dụ: `mmkv.clearAll()`.

Ví dụ:
```jsx
import { useState } from "react";
import { View, Text, Button } from "react-native";
import MMKV from 'react-native-mmkv';

const storage = new MMKV();

const App = () => {
  const [data, setData] = useState(null);

  const handleSetData = () => {
    storage.set('key', 'value');
  };

  const handleGetData = () => {
    const data = storage.getString('key');
    setData(data);
  };

  const handleRemoveData = () => {
    storage.delete('key');
  };

  return (
    <View>
      <Text>{data}</Text>
      <Button title="Set Data" onPress={handleSetData} />
      <Button title="Get Data" onPress={handleGetData} />
      <Button title="Remove Data" onPress={handleRemoveData} />
    </View>
  );
};
```